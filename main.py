...
async def ai_analyze(req: AnalyzeRequest):
    """Non-streaming AI threat analysis — returns full text at once."""
    client = _claude_client()
    ctx = _ip_context(req.result)
    det = req.result.get("detection", {})
    score = det.get("score", 0)

    system = f"""You are a cybersecurity analyst specializing in IP intelligence and threat assessment.
...
## Recommended action
Clear, specific action: block / monitor / allow / investigate.

Keep each section concise — 2-4 sentences. Plain professional language."""
...
async def ai_chat(req: ChatRequest):
    """Non-streaming chat reply about an IP."""
    client = _claude_client()
    ctx = _ip_context(req.result)
